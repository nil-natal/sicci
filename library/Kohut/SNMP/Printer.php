...
class Kohut_SNMP_Printer extends Kohut_SNMP_Abstract
{
     ...
     */
    const SNMP_PRINTER_FACTORY_ID                     = '.1.3.6.1.2.1.25.3.2.1.3.1';
    const SNMP_PRINTER_NAME_ID                        = '.1.3.6.1.4.1.11.2.4.3.1.27.1.6.1';    
    ...
    const SNMP_NUMBER_PAGE_COPY                       = '.1.3.6.1.4.1.11.2.3.9.4.2.1.2.2.1.63.0';
    const SNMP_TONER_LEVEL                            = '.1.3.6.1.2.1.43.11.1.1.9.1.1';
    ...
    const SNMP_TESTE                                  = '.1.3.6.1.4.1.11.2.3.9.4.2.1.1.16.4.1.1.2.0';
    const SNMP_TEMPO_LIGADO                           = '.1.3.6.1.4.1.11.2.4.3.4.9.0';
     ...
     * @return string|boolean
     */
    public function getTempoLigado()
    {
        return $this->getSNMPString(self::SNMP_TEMPO_LIGADO);
    }
    
     /**
     * Function gets sysName (name) of the printer,
     * or returns false if call failed
     *
     * @return string|boolean
     */
    ...
    {
        return $this->getSNMPString(self::SNMP_PRINTER_NAME_ID);
    }
    ...
    /**
     * Function gets TonerLevel name of printer
     *
     * @return string|boolean
     */
    public function getTonerlevel()
    {
        return $this->getSNMPString(self::SNMP_TONER_LEVEL);
    }
     ...
     * @return int|boolean
     */
    public function getNumberPageCopy()
    {
        snmp_set_quick_print(true);
        $getNumberPageCopy = $this->get(self::SNMP_NUMBER_PAGE_COPY);
        snmp_set_quick_print(false);

        return ($getNumberPageCopy !== false) ? (int) $getNumberPageCopy : false;
    }
    ...
    {
        snmp_set_quick_print(true);
        $getTeste = $this->get(self::SNMP_TESTE);
        snmp_set_quick_print(false);

        return ($getTeste !== false) ? (int) $getTeste : false;
    }
